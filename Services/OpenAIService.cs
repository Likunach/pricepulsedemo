    ...
    public class OpenAIService : IOpenAIService
    {
        ...
        public async Task<string> GetCompletionAsync(string prompt)
        {
            ...
            var requestBody = new
            {
                ...
                messages = new object[]
                {
                    new { role = "system", content = "You are a precise data extractor that returns only valid JSON with no extra text or code fences." },
                    new { role = "user", content = prompt }
                },
                max_tokens = 4000,
                temperature = 0.1
            ...
            var rawContent = responseObj?.Choices?.FirstOrDefault()?.Message?.Content ?? string.Empty;
            if (string.IsNullOrWhiteSpace(rawContent))
            {
                return string.Empty;
            }

            // Clean the content to ensure it is raw JSON
